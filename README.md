...
This project provides a demo of a heterogeneity-aware intermittent system that allows respective cores to be primarily engaged in computations and slow memory operations. Our design was integrated into FreeRTOS to realize an intermittent-aware multi-core operating system capable of concurrent task execution and instant system recovery. This project implements the system prototype that explores how to leverage heterogeneous cores to mitigate the progress impediment caused by heterogeneous memories and is integrated into the operating system on a Cypress device featuring heterogeneous dual cores and hybrid memories to conduct a series of experiments. In particular, a delegable and adaptive synchronization protocol is proposed to allow memory accesses to be delegated between cores and dynamically adapt to diverse memory access latency. Moreover, our design guarantees \emph{task serializability} across multiple cores and maintains data consistency despite frequent power failures.
